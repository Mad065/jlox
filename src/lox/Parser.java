...
class Parser {
    ...
    private Stmt breakStatement() {
        Token keyword = previous();
        consume(SEMICOLON, "Se esperaba ';' después de 'break'.");

        if (loopDepth == 0) {
            error(keyword, "No se puede usar 'break' fuera de un bucle.");
        }
    ...
    private Expr ternary() {
        ...
        if (match(QUESTION)) {
            ...
            expr = new Expr.Conditional(expr, previous(), thenBranch, previous(), elseBranch);
        }
    ...
    private Expr primary() {
        ...
        if (match(BANG_EQUAL, EQUAL_EQUAL)) {
            Lox.error(previous(), "Expect left operand before operator.");
            equality();
            return null;
        }
